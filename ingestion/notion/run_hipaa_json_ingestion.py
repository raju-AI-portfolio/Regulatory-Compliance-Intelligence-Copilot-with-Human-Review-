...
from ingestion.notion.hipaa_json_mapper import map_hipaa_structured_record
...
JSON_PATH = "data/structured/hipaa_structured.json"
NAMESPACE = "hipaa_structured"


def main():
    print(f"Loading structured HIPAA JSON from {JSON_PATH}")
    records = load_structured_json(JSON_PATH)
    print(f"Loaded {len(records)} structured records")

    mapped_chunks = []
    for record in records:
        metadata = map_hipaa_structured_record(record, namespace=NAMESPACE)
